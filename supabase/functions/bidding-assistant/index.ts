...
c) 字体：有明确要求的按要求执行，没有明确要求按文档模板执行，文档标题及页眉需匹配项目名称；`;
  ...
  // Build comprehensive scoring table text
  const scoringText = bid.scoring_table && Array.isArray(bid.scoring_table) && bid.scoring_table.length > 0
    ? JSON.stringify(bid.scoring_table)
    : "（无评分标准数据）";

  // Build disqualification items text
  const disqualText = bid.disqualification_items && Array.isArray(bid.disqualification_items) && bid.disqualification_items.length > 0
    ? bid.disqualification_items.map((d: any, i: number) => `${i + 1}. ${d.content || d.text || JSON.stringify(d)}`).join("\n")
    : "（无废标条目）";

  // Build trap items if available
  const trapText = bid.trap_items && Array.isArray(bid.trap_items) && bid.trap_items.length > 0
    ? bid.trap_items.map((t: any, i: number) => `${i + 1}. ${t.content || t.text || JSON.stringify(t)}`).join("\n")
    : "";

  const userContent = `【招标项目名称】${bid.project_name || "未命名"}
