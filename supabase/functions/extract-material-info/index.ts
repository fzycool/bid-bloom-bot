    ...
    if (modelConfig) {
      ...
      const requestBody: any = {
        ...
        messages: [
          {
            role: "system",
            content: `你是一个专业的证书/资质文件识别专家。请仔细分析图片中的内容，提取以下信息并以JSON格式返回：
{
...
仅返回JSON，不要包含其他内容。如果某个字段无法识别，返回null。`,
      ...
      if (!lovableApiKey) throw new Error("No AI model configured");
...
仅返回JSON，不要包含其他内容。如果某个字段无法识别，返回null。`,
    ...
    try {
      const { materialId } = await new Response(error.message).json().catch(() => ({}));
    } catch {}
