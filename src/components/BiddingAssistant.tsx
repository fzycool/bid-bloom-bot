...
c) 字体：有明确要求的按要求执行，没有明确要求按文档模板执行，文档标题及页眉需匹配项目名称；`;
  ...
  const fetchProposalDetails = useCallback(async (proposalId: string) => {
    ...
    const allSections = ((secs as any[]) || []).filter((s: any) => s.source_type !== "toc_generated" && s.source_type !== "toc_imported");
    ...
    // Auto-correct tocStatus: only if TOC entries (标书目录) exist, not outline sections (提纲)
                          ...
                          onAutoOrganize={async () => {
                            ...
                            try {
                              ...
                              for (const a of assignments) {
                                ...
                                } as any).eq("id", a.toc_entry_id);
                              }

                              await fetchProposalDetails(selectedProposal.id);
                              toast({ title: "目录整理完成", description: `已重新归类 ${assignments.length} 个目录条目` });
                            } catch (e: any) {
