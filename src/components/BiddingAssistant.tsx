...
c) 字体：有明确要求的按要求执行，没有明确要求按文档模板执行，文档标题及页眉需匹配项目名称；`;
  ...
  const fetchProposalDetails = useCallback(async (proposalId: string) => {
    ...
    const tocList = (tocData as TocEntry[]) || [];
    if (tocList.length > 0) {
                          ...
                          onReorder={async (items) => {
                            try {
                              ...
                              if (sectionItems.length > 0) {
                                updates.push(...sectionItems.map((item) =>
                                  supabase.from("proposal_sections").update({
                                    sort_order: item.sort_order,
                                    parent_id: item.parent_id,
                                  } as any).eq("id", item.id).then()
                                ));
                              }
                              
                              if (tocItems.length > 0) {
                                updates.push(...tocItems.map((item) =>
