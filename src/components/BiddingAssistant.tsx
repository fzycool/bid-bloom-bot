  ...
  if (selectedProposal) {
    ...
    if (workspaceMode) {
      ...
      const handleChatSend = async () => {
        ...
        setChatLoading(true);
        try {
          // Simple AI chat via edge function
          ...
          if (error) throw error;
          setChatMessages(prev => [...prev, { role: "assistant", content: data?.reply || "抱歉，暂时无法回复" }]);
        } catch (e: any) {
      ...
      const renderSectionContent = (content: string) => {
        ...
        // Source annotation patterns
        const sourceKbPattern = /【来源[：:]知识库\s*[-–—]\s*(.+?)(?:\s*[-–—]\s*(.+?))?】/g;
        const sourceAiPattern = /【来源[：:]AI智能生成】/g;

        // Render a line with inline source annotations highlighted
        const renderAnnotatedLine = (text: string, idx: number) => {
          ...
          while ((match = combinedPattern.exec(text)) !== null) {
            ...
            if (match[1].startsWith("AI")) {
              parts.push({ type: "ai", content: match[0] });
            } else {
              parts.push({ type: "kb", content: match[0], fileName: match[2]?.trim(), detail: match[3]?.trim() });
            }
          ...
          if (parts.length === 1 && parts[0].type === "text") {
            return <p key={idx} className="text-sm text-foreground/80 leading-relaxed">{text}</p>;
          }
          ...
          return (
            <p key={idx} className="text-sm text-foreground/80 leading-relaxed">
              {parts.map((part, pi) => {
                if (part.type === "kb") {
                  return (
                    <span key={pi} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-500/20 mx-0.5 whitespace-nowrap">
                      <FileText className="w-3 h-3" />
                      知识库: {part.fileName}{part.detail ? ` - ${part.detail}` : ""}
                    </span>
                  );
                }
                if (part.type === "ai") {
                  return (
                    <span key={pi} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-purple-500/10 text-purple-700 dark:text-purple-400 border border-purple-500/20 mx-0.5 whitespace-nowrap">
        ...
        return (
          <div className="space-y-1.5 mb-3">
            {lines.map((line, idx) => {
              ...
              // Check if it's a requirement/hint line
                ...
                return (
                  <div key={idx}>
                    {isRequirement ? (
                      ...
                      renderAnnotatedLine(trimmed.replace(imageUrlPattern, "").trim(), idx)
                    )}
              ...
              return renderAnnotatedLine(trimmed, idx);
            })}
          </div>
        );
      };
                      ...
                      sections.map((section) => (
                        <div key={section.id} id={`section-${section.id}`}>
                          ...
                          {section.children?.map((child) => (
                            <div key={child.id} className="ml-4 mt-3" id={`section-${child.id}`}>
                              <h3
                              ...
                              {child.content && renderSectionContent(child.content)}
                              {child.children?.map((sub) => (
                                <div key={sub.id} className="ml-4 mt-2" id={`section-${sub.id}`}>
                                  <h4
                                  ...
                                  {sub.content && renderSectionContent(sub.content)}
                                </div>
                              ))}
                            </div>
                          ))}
                    ...
                    {chatMessages.length === 0 && (
                      <div className="text-center py-12">
                        ...
                        <p className="text-xs text-muted-foreground">
                          选择左侧提纲章节，向AI提问或请求修改标书内容
                        </p>
                        <div className="mt-4 space-y-1.5">
                          {["帮我扩充这个章节的内容", "检查这部分是否符合招标要求", "优化这段文字的专业性"].map((hint) => (
                            <button
                              key={hint}
                ...
                <div className="p-3 border-t border-border shrink-0">
                  <div className="flex gap-2">
                    <Textarea
                      ...
                      rows={1}
                      placeholder={selectedSection ? `关于「${selectedSection.title}」提问...` : "选择章节后提问..."}
                      value={chatInput}
                ...
                {docStatus === "pending" && (
                  <Card className="flex flex-col items-center justify-center py-16">
                    ...
                    <p className="text-xs text-muted-foreground mb-6 max-w-md text-center">
                      根据应答提纲、知识库资料和证明材料，AI将为每个章节撰写详细的标书正文
                    </p>
                    <Button onClick={() => { handleGenerateProposal(); setWorkspaceMode(true); }} disabled={generatingDoc || sections.length === 0}>
                      {generatingDoc ? (
