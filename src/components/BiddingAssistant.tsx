...
c) 字体：有明确要求的按要求执行，没有明确要求按文档模板执行，文档标题及页眉需匹配项目名称；`;
  ...
  // Poll for proposal doc generation progress with stale detection
  useEffect(() => {
    if (!selectedProposal || docStatus !== "processing") return;
    let pollCount = 0;
    let lastProgress = "";
    let staleCount = 0;
    const MAX_STALE = 40; // 40 * 3s = 120s without progress change → auto-fail
    const interval = setInterval(async () => {
      const { data } = await supabase
        .from("bid_proposals")
      ...
      if (data) {
        ...
        if (d.proposal_doc_progress === lastProgress) {
          staleCount++;
          if (staleCount >= MAX_STALE) {
            console.warn("Proposal doc generation appears stale, auto-recovering");
            await supabase.from("bid_proposals").update({
              proposal_doc_status: "failed",
              proposal_doc_progress: "生成超时，后台函数可能已中断。请重新生成。",
            }).eq("id", selectedProposal.id);
            setDocStatus("failed");
            setDocProgress("生成超时，后台函数可能已中断。请重新生成。");
            clearInterval(interval);
            return;
          }
        } else {
          staleCount = 0;
          lastProgress = d.proposal_doc_progress || "";
        }
      }
